...
import (
	...
	phoenixchain_go_sdk "github.com/jiuliangLiu/phoenixchain-go-sdk"
...
func TestWeb3g_AdminNodeInfo(t *testing.T) {
	var expect = phoenixchain_go_sdk.NodeInfo{
	...
	var execFunc ExecFunc = func(geb3 Geb3) {
		...
		resp := phoenixchain_go_sdk.NodeInfo{}
...
func TestWeb3g_EstimateGas(t *testing.T) {
	...
	var execFunc ExecFunc = func(geb3 Geb3) {
		gas := uint64(1000)
		var req = phoenixchain_go_sdk.CallMsg2{
			From:     "atp1",
			To:       "atp2",
			Gas:      gas,
			GasPrice: big.NewInt(1000000000),
...
func TestWeb3g_GetLogs(t *testing.T) {
	...
	var execFunc ExecFunc = func(geb3 Geb3) {
		req := phoenixchain_go_sdk.FilterQuery{
			BlockHash: nil,
			FromBlock: nil,
			ToBlock:   nil,
			Addresses: nil,
...
func TestWeb3g_PlatonSign(t *testing.T) {
	...
	var respFunc = GenRespFunction(expect)
	var execFunc ExecFunc = func(geb3 Geb3) {
		req := phoenixchain_go_sdk.SignReq{
...
func TestWeb3g_Syncing(t *testing.T) {
	var expect = phoenixchain_go_sdk.SyncProgress{StartingBlock: 0, CurrentBlock: 10, HighestBlock: 10, PulledStates: 20, KnownStates: 30}
	var respFunc = GenRespFunction(expect)
	var execFunc ExecFunc = func(geb3 Geb3) {
		resp, _ := geb3.Syncing()
