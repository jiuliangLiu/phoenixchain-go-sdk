...
// to get platon protocol version by phoenixchain_protocolVersion
func (platon *PlatonRPC) ProtocolVersion() (uint64, error) {
	return platon.client.ProtocolVersion(ctx)
}

// to get syncing status of network by phoenixchain_syncing
...
// to get the storage of account at pos with key by phoenixchain_getStorageAt
// @param account an account on platon starts with 'atp' or 'atx'
...
// to get the code of account at pos with key by phoenixchain_getCode
// @param account an account on platon starts with 'atp' or 'atx'
// @param option     the block number as bignumber, or 'latest', 'pending', 'earliest'.
...
// to get nonce of account at pos with key by phoenixchain_getTransactionCount
// @param account an account on platon starts with 'atp' or 'atx'
// @param option     the block number as bignumber, or 'latest', 'pending', 'earliest'.
...
// to get transaction count by block hash by phoenixchain_getBlockTransactionCountByHash
...
// to get transaction count by block number  by phoenixchain_getBlockTransactionCountByNumber
// @param option     the block number as bignumber, or 'latest', 'pending', 'earliest'.
...
// to get block info by block hash  by phoenixchain_getBlockByHash
// @param hash  block hash
func (platon *PlatonRPC) BlockByHash(hash string) (string, error) {
	return platon.client.BlockByHash(ctx, hash)
}

// to get block info by block number  by phoenixchain_getBlockByNumber
// @param option     the block number as bignumber, or 'latest', 'pending', 'earliest'.
...
// to get transaction info by block hash by phoenixchain_getTransactionByHash
// @param hash  the block hash.
func (platon *PlatonRPC) TransactionByTxHash(hash common.Hash) (tx *ethclient.RpcTransaction, isPending bool, err error) {
	return platon.client.TransactionByTxHash(ctx, hash)
}

// to get transaction info by block hash and index by phoenixchain_getTransactionByBlockHashAndIndex
// @param hash  the block hash.
...
// to get transaction info by block number and index by phoenixchain_getTransactionByBlockNumberAndIndex
// @param option  the block number as bignumber, or 'latest', 'pending', 'earliest'.
// @param index the specific index
...
// to get transaction receipt by phoenixchain_getTransactionReceipt
...
// to get transaction receipt by phoenixchain_getLogs
...
// to call a contract by msg at option by phoenixchain_call,
...
// to send a transaction by phoenixchain_sendRawTransaction
// notice this msg is never mined to block.
// @param msg  the call message
// @param option  the block number as bignumber, or 'latest', 'pending', 'earliest'.
...
// to get proof of bls by phoenixchain_getSchnorrNIZKProve
...
// To check if the state has changed, call phoenixchain_getFilterChanges.
...
//Additionally Filters timeout when they aren't requested with phoenixchain_getFilterChanges for a period of time.
